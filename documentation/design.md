...
#### HU-5: Gestionar Roles (Sincronización de Usuarios)
...
classDiagram
    ...
    class GithubClientImpl {
        +getOrganizationMembers() List<GithubUser>
    }
    class User {
        -Long githubId
        -String username
...
Este diagrama muestra los componentes principales de la funcionalidad de sincronización de usuarios.

*   **`UserSyncService`**: Es el orquestador central. Depende de dos interfaces, `GithubUserCollector` para obtener los usuarios de una fuente externa y `UserRepository` para interactuar con la base de datos local, siguiendo el Principio de Inversión de Dependencias.
*   **`GithubClientImpl`**: Es la implementación concreta que se comunica con la API de GitHub.
*   **`UserRepository`**: Es la interfaz para las operaciones de base de datos sobre la entidad `User`.
*   **`User`**: Es la entidad de dominio que representa a un usuario en nuestro sistema.

##### Diagrama de Base de Datos (Entidad `User`)
...
erDiagram
    USERS {
        ...
        varchar avatar_url "URL del avatar"
        boolean active "Indica si el usuario está activo"
...
Este ERD muestra la estructura de la tabla `USERS`.

*   Almacena información esencial de GitHub como `github_id`, `username`, y `avatar_url`.
*   El campo booleano `active` es crucial. Permite al sistema "desactivar" usuarios sin perder su información histórica, lo cual fue un requisito clave que probamos.
*   Se incluyen los timestamps `created_at` y `updated_at` para fines de auditoría.

##### Diagrama de Secuencia

```mermaid
sequenceDiagram
    participant CronJob as "Scheduler (CronJob)"
    participant Service as "UserSyncService"
    participant Github as "GithubClient"
    participant DB as "UserRepository"

    CronJob->>Service: synchronizeUsers()
    Service->>Github: getOrganizationMembers()
    activate Github
    Github-->>Service: Devuelve List<GithubUser>
    deactivate Github

    Service->>DB: findAll()
    activate DB
    DB-->>Service: Devuelve List<User> (locales)
    deactivate DB

    loop Para cada GithubUser de GitHub
        Service->>Service: ¿Existe en la BD local?
        alt Si no existe (Usuario Nuevo)
            Service->>Service: Crea un nuevo objeto User (activo)
            Note right of Service: Añade a la lista 'usersToSave'
        else Si ya existe (Usuario Existente)
            Service->>Service: Actualiza datos (ej. avatar_url)
            Note right of Service: Añade a la lista 'usersToSave'
        end
    end

    loop Para cada User local de la BD
        Service->>Service: ¿Está en la lista de GitHub?
        alt No está y está activo
            Service->>Service: Marca el usuario como inactivo (localUser.setActive(false))
            Note right of Service: Añade a la lista 'usersToSave'
        end
    end

    alt Si la lista 'usersToSave' no está vacía
        Service->>DB: saveAll(usersToSave)
        activate DB
        DB-->>Service: Devuelve usuarios guardados
        deactivate DB
    end
```
**Explicación del Diagrama:**

Este diagrama detalla el flujo de trabajo del proceso `synchronizeUsers`.

1.  **Disparo:** El proceso es iniciado por un job programado (`CronJob`).
2.  **Obtener Datos Externos:** `UserSyncService` llama al `GithubClient` para obtener la lista actualizada de todos los miembros de la organización de GitHub.
3.  **Obtener Datos Locales:** El servicio luego llama al `UserRepository` para obtener la lista de todos los usuarios actualmente almacenados en la base de datos local.
4.  **Comparación y Lógica:** El servicio itera sobre ambas listas:
    *   Crea nuevos objetos `User` para los miembros encontrados en GitHub pero no en la BD local.
    *   Actualiza los datos (ej. `avatar_url`) para los usuarios que se encuentran en ambas listas.
    *   Marca a los usuarios locales como `inactivos` si ya no están presentes en la lista de GitHub.
5.  **Guardado Condicional:** De manera crucial, el servicio solo llama a `saveAll` en el `UserRepository` si la lista de usuarios a crear, actualizar o desactivar (`usersToSave`) no está vacía. Esto evita escrituras innecesarias en la base de datos, un punto clave de nuestras pruebas.

...
class DeploymentTest {
    ...
    void testAllArgsConstructor() {
        ...
        // When
        Deployment deployment = new Deployment(githubId, name, headBranch, status, conclusion, now, now);
    ...
    void testEqualsAndHashCode() {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Deployment deployment1 = new Deployment(123L, "name", "branch", "status", "conclusion", now, now);
        deployment1.setId(1L);

        Deployment deployment2 = new Deployment(123L, "name", "branch", "status", "conclusion", now, now);
        deployment2.setId(1L);

        Deployment deployment3 = new Deployment(456L, "another-name", "another-branch", "another-status", "another-conclusion", now, now);
        deployment3.setId(2L);

        Deployment deployment4 = new Deployment(123L, "name", "branch", "status", "conclusion", now, now);
        deployment4.setId(3L); // Same githubId, but different id

        Deployment deployment5 = new Deployment(456L, "name", "branch", "status", "conclusion", now, now);
    ...
    void testEquals_sameInstance_returnsTrue() {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Deployment deployment = new Deployment(123L, "name", "branch", "status", "conclusion", now, now);
        deployment.setId(1L);
